...
template class CuckooMap<uint64_t, std::shared_ptr<EphemeralPool::OverwriteVersionHeader>>;

template class CuckooMap<cid_t, std::vector<TxnVersionInfo>>;
template class CuckooMap<uint64_t, std::vector<std::pair<cid_t, cid_t>>>;

//// Used in Catalog::Manager
//template class CuckooMap<oid_t , std::shared_ptr<VersionBlock>>;
//
//// Used in Shared Pointer test and iterator test
//template class CuckooMap<oid_t , std::shared_ptr<oid_t>>;
//
//template class CuckooMap<cid_t , std::pair<GCVersionType, std::pair<void *, uint32_t>>>;
