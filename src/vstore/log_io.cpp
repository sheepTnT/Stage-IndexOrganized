#include "../include/vstore/log_io.h"
...
void BufferedLogReader::RefillBuffer() {
    PELOTON_ASSERT(read_head_ == filled_size_,"Refill Buffer fail.");
  if (in_ == -1) throw std::runtime_error("No more bytes left in the log file");
  read_head_ = 0;
  filled_size_ = PosixIoWrappers::ReadFully(in_, buffer_, LOG_BUFFER_SIZE);
  if (filled_size_ < LOG_BUFFER_SIZE) {
