...
#ifndef MVSTORE_SYNC_H
#define MVSTORE_SYNC_H
...
public:
    ...
    RefManager(size_t max_num_threads) : thread_refs(max_num_threads, nullptr),
...
template<int buckets>
thread_local uint64_t DistributedCounter<buckets>::cpuId = (uint64_t) std::hash<std::thread::id>{}(
