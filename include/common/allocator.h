...
struct AllocationUtil {
   ...
   */
  static char *AllocateAligned(uint64_t byte_size) {
    ...
    return reinterpret_cast<char *>(new uint64_t[(byte_size + 7) / 8]);
  }
...
class ByteAlignedAllocator {
 public:
   ...
   */
  void Delete(T *const ptr) { delete[] reinterpret_cast<char *>(ptr); }  // NOLINT
