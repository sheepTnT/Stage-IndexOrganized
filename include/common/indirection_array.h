...
class IndirectionArray {
 ...
 private:
  typedef std::array<LSN_T, INDIRECTION_ARRAY_MAX_SIZE> indirection_array_t;
...
class RecordIndirectionArray {

public:
    RecordIndirectionArray(oid_t oid) : oid_(oid) {
        indirections_.reset(new indirection_array_t());
    }

    ~RecordIndirectionArray() {}
    ...
    RecordLocation *GetIndirectionByOffset(const size_t &offset) {
        return &(indirections_->at(offset));
    }
...
private:
    typedef std::array<RecordLocation, INDIRECTION_ARRAY_MAX_SIZE> indirection_array_t;
