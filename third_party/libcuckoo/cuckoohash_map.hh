...
private:
    ...
    // index is in the table or not. It uses aligned_storage arrays to vstore the
...
private:
    ...
    // b_queue is the queue used to vstore b_slots for BFS cuckoo hashing.
    class b_queue {
    ...
    bool cuckoopath_move(const size_t hp, CuckooRecords& cuckoo_path,
        ...
        while (depth > 0) {
            ...
            if (depth == 1) {
                ...
                // must be locked. We vstore tb inside the extrab container so it
    ...
    // try_read_from_bucket will search the bucket for the given key and vstore
    ...
    // try_find_insert_bucket will search the bucket and vstore the index of an
